...
export class InboundParsing {
    ...
    async getInboundEmailEvents(
        ...
        const operationUrl: string = baseURL.replace(/\/$/, "") + "/inbound/events";
        const client: AxiosInstance = this.sdkConfiguration.defaultClient;
        let globalSecurity = this.sdkConfiguration.security;
        if (typeof globalSecurity === "function") {
            globalSecurity = await globalSecurity();
            ...
            case httpRes?.status == 200:
                if (utils.matchContentType(contentType, `application/json`)) {
                    res.getInboundEmailEvents = utils.objectToClass(
                        ...
                        components.GetInboundEmailEvents
                    );
                } else {
                    throw new errors.SDKError(
                        "unknown content-type received: " + contentType,
    ...
    async getInboundEmailEventsByUuid(
        ...
        const operationUrl: string = utils.generateURL(baseURL, "/inbound/events/{uuid}", req);
        const client: AxiosInstance = this.sdkConfiguration.defaultClient;
        let globalSecurity = this.sdkConfiguration.security;
        if (typeof globalSecurity === "function") {
            globalSecurity = await globalSecurity();
            ...
            case httpRes?.status == 200:
                if (utils.matchContentType(contentType, `application/json`)) {
                    res.getInboundEmailEventsByUuid = utils.objectToClass(
                        JSON.parse(decodedRes),
                        components.GetInboundEmailEventsByUuid
                    );
                } else {
                    throw new errors.SDKError(
                        "unknown content-type received: " + contentType,
